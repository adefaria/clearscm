...
try {
    ...
    echo "Checking if function 'inotify_read' exists...<br>";
    if (function_exists('inotify_read')) {
        echo "function_exists('inotify_read') returns <b>TRUE</b>.<br>";
    } else {
        echo "function_exists('inotify_read') returns <b>FALSE</b>.<br>";
        throw new Exception("inotify_readis not defined according to function_exists(). This is the core problem.");
    }

    echo "Attempting to call inotify_read() with IN_NONBLOCK...<br>";
    // Trigger an event
    file_put_contents($tempFile, "some data " . time() . "\n");
    echo "Modified temporary file to trigger event.<br>";

    $events = inotify_read($fd, IN_NONBLOCK);

    if ($events === false) {
        echo "inotify_read() returned false. For IN_NONBLOCK, this usually means no events were immediately pending (which might be okay if event queue processing is too fast) or an error occurred.<br>";
        // To be more certain, let's try a blocking read for a very short time
        // echo "Attempting a short blocking read...<br>";
        // $events_blocking = inotify_read($fd); // This will block
        ...
        echo "inotify_read() successful. Events received: <pre>" . htmlspecialchars(print_r($events, true)) . "</pre><br>";
    } else {
        echo "inotify_read() returned an unexpected value: " . htmlspecialchars(var_export($events, true)) . "<br>";
    }

    echo "<p style='color:green;'>Test appears to have passed the inotify_read() call point.</p>";
